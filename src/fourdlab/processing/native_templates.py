...
class NativeCifZonePreview:
    ...
    zone_label: str
    zone_barycentric: np.ndarray | None = None
    zone_color_rgb: np.ndarray | None = None
...
def generate_cif_zone_preview(
    ...
    *,
    ...
    if hkl.shape[0] == 0:
        return _empty_preview(axis_cart, basis_x, basis_y, points, float(k_max), axis_lattice)
    ...
    return NativeCifZonePreview(
        ...
        matrix=matrix,
        zone_axis_lattice=axis_lattice,
        zone_axis_cart=axis_cart,
        zone_label=_format_zone_label(axis_lattice) if zone_label is None else zone_label,
        zone_barycentric=None if zone_barycentric is None else np.asarray(zone_barycentric, dtype=np.float64),
        zone_color_rgb=None if zone_color_rgb is None else np.asarray(zone_color_rgb, dtype=np.float64),
    )
...
def _empty_preview(
    axis_cart: np.ndarray,
    basis_x: np.ndarray,
    basis_y: np.ndarray,
    points: int,
    k_max: float,
    axis_lattice: np.ndarray | None = None,
) -> NativeCifZonePreview:
    q = np.linspace(0.0, k_max, max(2, int(points)))
    lattice = axis_cart if axis_lattice is None else axis_lattice
    return NativeCifZonePreview(
        ...
        matrix=np.column_stack((basis_x, basis_y, axis_cart)),
        zone_axis_lattice=lattice,
        zone_axis_cart=axis_cart,
        zone_label=_format_zone_label(lattice),
        zone_barycentric=None,
        zone_color_rgb=None,
    )


def triangle_zone_samples(
    vertices: tuple[
    ...
    """Sample lattice zone axes inside a triangle using barycentric weights."""
    ...
    colors = np.asarray(
        [[0.95, 0.12, 0.16], [0.10, 0.58, 0.95], [0.16, 0.72, 0.28]],
        dtype=np.float64,
    )
    labels = [_format_zone_label(vertex) for vertex in verts]
    ...
    for i in range(n + 1):
        for j in range(n + 1 - i):
            k = n - i - j
            bary = np.asarray([i / n, j / n, k / n], dtype=np.float64)
            axis = _normalize(bary @ verts)
            key = tuple(np.rint(axis * 1.0e6).astype(int).tolist())
            if key in seen:
                continue
            seen.add(key)
            color = bary @ colors
            vertex_hit = np.flatnonzero(np.isclose(bary, 1.0))
            label = labels[int(vertex_hit[0])] if vertex_hit.size else _format_zone_label(axis)
            samples.append((axis, bary, color, label))
    # Ensure exact vertices are present and have exact labels/colors.
    for idx in range(3):
        bary = np.zeros(3, dtype=np.float64)
        bary[idx] = 1.0
        axis = _normalize(verts[idx])
        key = tuple(np.rint(axis * 1.0e6).astype(int).tolist())
        if key not in seen:
            samples.append((axis, bary, colors[idx], labels[idx]))
            seen.add(key)
    return samples
