...
from fourdlab.processing.native_calibration import (
    ...
    radial_profile_from_peak_list,
    run_radial_cif_calibration,
    run_vector_cif_calibration,
)
from fourdlab.processing.native_orientation import (
    NativeOrientationCancelled,
    NativeOrientationConfig,
    NativeOrientationMapResult,
    NativeOrientationPlan,
    build_native_orientation_plan,
    in_plane_angle_to_rgb,
    match_native_orientation,
    native_display_array,
    orientation_color_image_native,
    run_native_orientation_map,
)
...
class DiffractionAnalysisWindow(QMainWindow):
    ...
    def __init__(self, viewer) -> None:
        ...
        self.orientation_radial_calibrations: dict[str, RadialCalibrationResult] = {}
        self.orientation_vector_calibrations: dict[str, VectorCalibrationResult] = {}
    ...
    def _build_peak_tab(self) -> QWidget:
        ...
        self.preview.getView().setMenuEnabled(False)
    ...
    def _build_orientation_tab(self, mode: str) -> QWidget:
        ...
        method_group = QGroupBox("Input Peaks and CIF / Crystal")
        ...
        calibration_group = QGroupBox("CIF Calibration")
        ...
        detector_rotation = _double_box(0.0, -180.0, 180.0, 0.25)
        match_tolerance_q = _double_box(0.04, 1.0e-6, 10.0, 0.005)
        match_tolerance_q.setDecimals(6)
        center_exclusion_q = _double_box(0.0, 0.0, 10.0, 0.005)
        center_exclusion_q.setDecimals(6)
        ...
        add_row(calibration_form, "detector rotation (deg)", detector_rotation)
        add_row(calibration_form, "match tolerance q", match_tolerance_q)
        add_row(calibration_form, "center exclusion q", center_exclusion_q)
        add_row(calibration_form, flip_qy)

        cif_model_group = QGroupBox("CIF Calibration Workflow")
        ...
        q_pixel_source.addItems(["Full BVM", "Current scan pixel", "Selected ROI average"])
        ...
        preview_cif = QPushButton("Preview CIF Zone")
        preview_cif.clicked.connect(lambda: self.preview_cif_zone(mode))
        build_bvm = QPushButton("Build Experimental BVM")
        build_bvm.clicked.connect(lambda: self.show_centered_bvm(mode))
        scan_pixel = QPushButton("Run Radial Calibration")
        scan_pixel.clicked.connect(lambda: self.scan_orientation_q_pixel(mode))
        vector_calibration = QPushButton("Run Vector Calibration")
        vector_calibration.clicked.connect(lambda: self.run_vector_calibration(mode))
        apply_calibration = QPushButton("Apply Calibration")
        apply_calibration.clicked.connect(lambda: self.apply_vector_calibration(mode))
        diagnose_fit = QPushButton("Show Calibration Report")
        diagnose_fit.clicked.connect(lambda: self.show_calibration_report(mode))
        calibration_report = QTextEdit()
        calibration_report.setReadOnly(True)
        calibration_report.setMinimumHeight(110)
        ...
        add_row(cif_model_form, build_bvm)
        add_row(cif_model_form, scan_pixel)
        add_row(cif_model_form, vector_calibration)
        add_row(cif_model_form, apply_calibration)
        add_row(cif_model_form, diagnose_fit)
        add_row(cif_model_form, "Calibration report", calibration_report)
        ...
        match_group = QGroupBox("Orientation Library and Matching")
        ...
        gpu_chunk = _int_box(4096, 256, 65536)
        rotation_symmetry = _int_box(1, 1, 12)
        angle_convention = QComboBox()
        angle_convention.addItems(["CIF-positive", "Detector-screen-positive", "Invert sign"])
        angle_convention.currentIndexChanged.connect(lambda _value: self._rotation_symmetry_changed(mode))
        zero_angle_offset = _double_box(0.0, -360.0, 360.0, 1.0)
        zero_angle_offset.valueChanged.connect(lambda _value: self._rotation_symmetry_changed(mode))
        uniform_template_intensity = QCheckBox("Use uniform template intensity")
        use_cuda = QCheckBox("Use GPU")
        ...
        add_row(match_form, "GPU template chunk", gpu_chunk)
        add_row(match_form, "Rotation symmetry", rotation_symmetry)
        add_row(match_form, "Angle convention", angle_convention)
        add_row(match_form, "Zero angle offset", zero_angle_offset)
        add_row(match_form, uniform_template_intensity)
        ...
        save_data.clicked.connect(lambda: self.save_orientation_data(mode))
        display = QComboBox()
        display.addItems(
            [
                "Orientation color",
                "Zone-axis triangle color",
                "Best score",
                "Second score",
                "Confidence gap",
                "Confidence ratio",
        ...
        if mode == "out_of_plane":
            display.setCurrentText("Zone-axis triangle color")
        display.currentIndexChanged.connect(lambda: self.refresh_orientation_display(mode))
        auto_levels = QCheckBox("Auto contrast")
        auto_levels.setChecked(True)
        min_level = _double_box(0.0, -1.0e12, 1.0e12, 0.1)
        max_level = _double_box(1.0, -1.0e12, 1.0e12, 0.1)
        min_level.setDecimals(6)
        max_level.setDecimals(6)
        apply_levels = QPushButton("Apply levels")
        apply_levels.clicked.connect(lambda: self.apply_orientation_levels(mode))
        reset_levels = QPushButton("Reset levels")
        reset_levels.clicked.connect(lambda: self.reset_orientation_levels(mode))
        color_wheel = QLabel()
        ...
        levels_layout.addWidget(auto_levels)
        levels_layout.addWidget(QLabel("Min"))
        levels_layout.addWidget(min_level)
        levels_layout.addWidget(QLabel("Max"))
        levels_layout.addWidget(max_level)
        result_layout.addWidget(levels_row)
        result_layout.addWidget(apply_levels)
        result_layout.addWidget(reset_levels)
        result_layout.addWidget(save_image)
        result_layout.addWidget(save_data)
        result_layout.addWidget(color_wheel)
        triangle_legend = QLabel()
        triangle_legend.setAlignment(Qt.AlignCenter)
        if mode != "out_of_plane":
            triangle_legend.hide()
        result_layout.addWidget(triangle_legend)

        map_toolbar = QWidget()
        map_toolbar_layout = QHBoxLayout(map_toolbar)
        map_toolbar_layout.setContentsMargins(0, 0, 0, 0)
        pan_tool = QPushButton("Pan/Zoom")
        pan_tool.setCheckable(True)
        pan_tool.setChecked(True)
        inspect_tool = QPushButton("Inspect Pixel")
        inspect_tool.setCheckable(True)
        toolbar_auto = QPushButton("Auto Contrast")
        toolbar_auto.clicked.connect(lambda: self.reset_orientation_levels(mode))
        toolbar_apply = QPushButton("Apply Manual Levels")
        toolbar_apply.clicked.connect(lambda: self.apply_orientation_levels(mode))
        pan_tool.clicked.connect(lambda: self._set_orientation_tool(mode, "pan"))
        inspect_tool.clicked.connect(lambda: self._set_orientation_tool(mode, "inspect"))
        map_toolbar_layout.addWidget(pan_tool)
        map_toolbar_layout.addWidget(inspect_tool)
        map_toolbar_layout.addStretch(1)
        map_toolbar_layout.addWidget(toolbar_auto)
        map_toolbar_layout.addWidget(toolbar_apply)

        image = pg.ImageView()
        image.ui.roiBtn.hide()
        image.ui.menuBtn.hide()
        image.ui.histogram.show()
        image.getView().setMenuEnabled(False)
        ...
        image.getView().addItem(marker)
        image.getView().scene().sigMouseClicked.connect(
            lambda event, m=mode: self._orientation_map_clicked(m, event)
        )
        plot = pg.PlotWidget()
        ...
        layout.addWidget(left_scroll, 0, 0, 2, 1)
        result_area = QWidget()
        result_area_layout = QGridLayout(result_area)
        result_area_layout.setContentsMargins(0, 0, 0, 0)
        result_area_layout.addWidget(map_toolbar, 0, 0, 1, 2)
        image_panel = _aspect_panel(image, aspect=2.05) if mode == "out_of_plane" else _square_panel(image)
        result_area_layout.addWidget(image_panel, 1, 0, 1, 1)
        result_area_layout.addWidget(_square_panel(plot, min_size=260), 1, 1, 1, 1)
        result_area_layout.setColumnStretch(0, 3)
        result_area_layout.setColumnStretch(1, 2)
        result_area_layout.setRowStretch(1, 1)
        layout.addWidget(result_area, 0, 1, 2, 1)
        ...
        self.orientation_controls[mode] = {
            ...
            "match_tolerance_q": match_tolerance_q,
            "center_exclusion_q": center_exclusion_q,
            "k_max": k_max,
            ...
            "uniform_template_intensity": uniform_template_intensity,
            ...
            "triangle_legend": triangle_legend,
    ...
    def preview_cif_zone(self, mode: str) -> None:
        ...
        try:
            preview = generate_cif_zone_preview(
                ...
                sigma_excitation=controls["sigma_excitation"].value(),
            )
        except Exception as exc:
    ...
    def scan_orientation_q_pixel(self, mode: str) -> None:
        ...
        if source_name == "Current scan pixel":
            ...
            if peaks is None:
                ...
                return
            radial_source = radial_profile_from_peak_list(
                peaks,
                qx_center=controls["qx_center"].value(),
                qy_center=controls["qy_center"].value(),
            )
        ...
        if crystal is None:
            return
        try:
            scan = run_radial_cif_calibration(
                radial_source,
                crystal,
                self._cif_calibration_config(mode),
                pixel_min=controls["q_pixel_min"].value(),
                pixel_max=controls["q_pixel_max"].value(),
                steps=controls["q_pixel_steps"].value(),
            )
        except Exception as exc:
            self._orientation_log(mode, f"radial calibration failed: {exc}")
            return
        self.orientation_radial_calibrations[mode] = scan
        controls["q_pixel_size"].setValue(scan.best_q_pixel_size)
        ...
        self._orientation_log(
            mode,
            f"Best q pixel size from {source_name} radial calibration{suffix}: "
            f"{scan.best_q_pixel_size:.6g} A^-1/pixel",
        )

    def run_vector_calibration(self, mode: str) -> None:
        if self._is_no_cif_mode(mode):
            self._orientation_log(mode, "Vector calibration is available in CIF indexing mode.")
            return
        if mode == "out_of_plane":
            self._sync_out_of_plane_if_requested()
        controls = self.orientation_controls[mode]
        ...
        if self.all_peaks is not None:
            y, x = self._viewer_scan_position()
            sample = [self.all_peaks.peaks[y][x]]
            sample.extend(
                self.all_peaks.peaks[yy][xx]
                for yy in range(self.all_peaks.scan_shape[0])
                for xx in range(self.all_peaks.scan_shape[1])
                if (yy, xx) != (y, x) and len(sample) < 9
            )
            peaks: PeakList | list[PeakList] = sample
        else:
            ...
            peaks = current
        try:
            result = run_vector_cif_calibration(
                peaks,
                crystal,
                self._cif_calibration_config(mode),
            )
        except Exception as exc:
            self._orientation_log(mode, f"Vector calibration failed: {exc}")
            return
        self.orientation_vector_calibrations[mode] = result
        self._update_calibration_report(mode)
        self._orientation_log(
            mode,
            f"Vector calibration {result.quality}: matched {result.matched_peak_count}, "
            f"median residual {result.median_residual_q:.4g} A^-1.",
        )

    def apply_vector_calibration(self, mode: str) -> None:
        result = self.orientation_vector_calibrations.get(mode)
        if result is None:
            self._orientation_log(mode, "Run vector calibration before applying it.")
            return
        controls = self.orientation_controls[mode]
        controls["qx_center"].setValue(result.qx_center)
        controls["qy_center"].setValue(result.qy_center)
        controls["q_pixel_size"].setValue(result.q_pixel_size)
        controls["flip_qy"].setChecked(result.flip_qy)
        controls["detector_rotation"].setValue(result.detector_rotation_deg)
        controls["match_tolerance_q"].setValue(result.recommended_match_tolerance_q)
        self.orientation_workspaces.pop(mode, None)
        self.orientation_results.pop(mode, None)
        self._update_calibration_report(mode)
        self._orientation_log(mode, "Applied vector calibration to CIF indexing controls.")

    def show_calibration_report(self, mode: str) -> None:
        self._update_calibration_report(mode)

    def diagnose_cif_fit(self, mode: str) -> None:
        self.show_calibration_report(mode)
    ...
    def build_orientation_plan(self, mode: str) -> None:
        ...
        controls = self.orientation_controls[mode]
        try:
            config = self._native_orientation_config(mode)
            crystal = self._orientation_crystal(mode)
            if crystal is None:
                return
            controls["progress"].setRange(0, 0)
            controls["progress"].setFormat("Building native orientation plan...")
            workspace = build_native_orientation_plan(crystal, config)
        except Exception as exc:
        ...
        self._orientation_log(
            mode,
            f"Native orientation plan ready for {mode.replace('_', '-')} "
            f"with {len(workspace.templates)} templates.",
        )
        if workspace.config.use_gpu:
            if workspace.gpu_bank is None:
                self._orientation_log(mode, "GPU requested but unavailable or preparation failed; fast CPU matching will be used.")
            else:
                self._orientation_log(mode, f"GPU template bank ready; chunk size {workspace.config.gpu_chunk_size}.")
    ...
    def preview_current_orientation(self, mode: str) -> None:
        ...
        if peaks is None:
            ...
            return
        try:
            matches = match_native_orientation(workspace, peaks)
        except Exception as exc:
            self._orientation_log(mode, f"Preview orientation failed: {exc}")
            return
        if not matches:
            self._orientation_log(mode, "Preview orientation found no valid native match.")
            return
        template_idx, match = matches[0]
        template = workspace.templates[template_idx]
        self._orientation_log(
            mode,
            "Current orientation: "
            f"score {match.score:.4g}, angle {template.in_plane_angle_deg:.2f} deg, "
            f"matched {match.matched_peak_count}, mean residual {match.mean_residual_q:.4g} A^-1",
        )
        self._show_native_fit_overlay(mode, workspace, peaks, matches)
    ...
    def show_fit_preview(self, mode: str) -> None:
        ...
        if isinstance(result, NativeOrientationMapResult) and y < result.score_topn.shape[0] and x < result.score_topn.shape[1]:
            matches = self._native_matches_from_result_pixel(workspace, result, y, x)
        if matches is None:
            try:
                matches = match_native_orientation(workspace, peaks)
            except Exception as exc:
                ...
                return
        if not matches:
            self._orientation_log(mode, f"Pixel inspector at ({y}, {x}) found no valid native match.")
            return
        self._show_native_fit_overlay(mode, workspace, peaks, matches)
        best = matches[0][1]
        second = matches[1][1].score if len(matches) > 1 else float("nan")
        gap = best.score - (second if np.isfinite(second) else 0.0)
        self._orientation_log(
            mode,
            f"Pixel inspector ({y}, {x}): best {best.score:.4g}, second {second:.4g}, "
            f"gap {gap:.4g}, matched {best.matched_peak_count}, residual {best.mean_residual_q:.4g}.",
        )
    ...
    def run_orientation_map(self, mode: str) -> None:
        ...
        def progress(done: int, total: int) -> None:
            ...
            controls["progress"].setFormat(f"Native matching {done}/{total}")

        def task(progress_emit, stop_requested):
            return run_native_orientation_map(
                workspace,
                peaks,
                progress=progress_emit,
                stop_requested=stop_requested,
        ...
        def finished(result) -> None:
            ...
            if controls.get("selected_pixel") is not None:
                self._update_orientation_inspector(mode)
            self._orientation_log(
                mode,
                f"Native orientation map complete. best-score range "
                f"{np.nanmin(result.best_score):.4g}..{np.nanmax(result.best_score):.4g}",
            )
        ...
        self._orientation_task = start_background_task(
            self,
            task,
            cancelled_exception=NativeOrientationCancelled,
            on_progress=progress,
            on_finished=finished,
            on_failed=failed,
            on_cancelled=cancelled,
    ...
    def save_orientation_data(self, mode: str) -> None:
        result = self.orientation_results.get(mode)
        if result is None:
            self._orientation_log(mode, "No orientation result to save.")
            return
        path, _ = QFileDialog.getSaveFileName(
            self,
            "Save orientation data",
            str(_project_root() / f"{mode}_native_orientation.npz"),
        ...
        try:
            if isinstance(result, NativeOrientationMapResult):
                np.savez_compressed(
                    out,
                    score_topn=result.score_topn,
                    matrix_topn=result.matrix_topn,
                    zone_axis_topn=result.zone_axis_topn,
                    zone_axis_lattice_topn=result.zone_axis_lattice_topn,
                    zone_axis_cart_topn=result.zone_axis_cart_topn,
                    zone_barycentric_topn=result.zone_barycentric_topn,
                    zone_color_rgb_topn=result.zone_color_rgb_topn,
                    zone_label_topn=result.zone_label_topn,
                    in_plane_angle_topn_deg=result.in_plane_angle_topn_deg,
                    matched_peak_count_topn=result.matched_peak_count_topn,
                    mean_residual_topn=result.mean_residual_topn,
                    template_index_topn=result.template_index_topn,
                    ...
                    mean_residual_q=result.mean_residual_q,
                    valid_mask=result.valid_mask,
                    best_matrix=result.best_matrix,
                    best_zone_axis=result.best_zone_axis,
                    best_zone_axis_lattice=result.best_zone_axis_lattice,
                    best_zone_axis_cart=result.best_zone_axis_cart,
                    best_zone_color_rgb=result.best_zone_color_rgb,
                    best_zone_barycentric=result.best_zone_barycentric,
                    in_plane_angle_deg=result.in_plane_angle_deg,
                    tilt_deg=result.tilt_deg,
                    tilt_azimuth_deg=result.tilt_azimuth_deg,
                )
            else:
                np.savez_compressed(
                    out,
            ...
            self._orientation_log(mode, f"Failed to save orientation data: {exc}")
            return
        self._orientation_log(mode, f"Saved orientation data: {out}")
    ...
    def refresh_orientation_display(self, mode: str) -> None:
        ...
        if isinstance(result, NativeOrientationMapResult):
            if label in {"Orientation color", "Zone-axis triangle color"}:
                image = orientation_color_image_native(
                    result,
                    mode,
                    symmetry_order=self._rotation_symmetry(mode),
                    angle_sign=self._native_angle_sign(mode),
                    zero_angle_offset_deg=controls["zero_angle_offset"].value(),
                )
                controls["image"].setImage(np.transpose(image, (1, 0, 2)), autoLevels=False)
                controls["plot_kind"] = "image"
                self._add_analysis_result(
                    AnalysisResult(
                        key=f"{mode}_native_orientation_color",
                        title="Native Orientation Color",
                        kind="orientation_map",
                        image=image,
                    )
                )
                return
            image = native_display_array(result, label)
            self._set_orientation_scalar_image(mode, image, label)
            controls["plot_kind"] = "image"
            return
    ...
    def _set_orientation_scalar_image(self, mode: str, image: np.ndarray, label: str) -> None:
        """Display a scalar scan_y/scan_x map; transpose is only for ImageView axes."""

        controls = self.orientation_controls[mode]
        display_image = np.asarray(image, dtype=np.float64).T
        if controls["auto_levels"].isChecked():
            controls["image"].setImage(display_image, autoLevels=True)
            levels = _levels(np.asarray(image, dtype=np.float64))
            controls["min_level"].setValue(levels[0])
            controls["max_level"].setValue(levels[1])
        else:
            low = controls["min_level"].value()
            high = controls["max_level"].value()
            if high <= low:
                high = low + 1.0
                controls["max_level"].setValue(high)
            controls["image"].setImage(display_image, autoLevels=False)
            controls["image"].setLevels(low, high)
            controls["level_memory"][label] = (low, high)
        self._update_orientation_marker(mode)

    def apply_orientation_levels(self, mode: str) -> None:
        controls = self.orientation_controls[mode]
        controls["auto_levels"].setChecked(False)
        label = controls["display"].currentText()
        controls["level_memory"][label] = (controls["min_level"].value(), controls["max_level"].value())
        self.refresh_orientation_display(mode)

    def reset_orientation_levels(self, mode: str) -> None:
        controls = self.orientation_controls[mode]
        label = controls["display"].currentText()
        controls["level_memory"].pop(label, None)
        controls["auto_levels"].setChecked(True)
        self.refresh_orientation_display(mode)

    def _set_orientation_tool(self, mode: str, tool: str) -> None:
        controls = self.orientation_controls[mode]
        controls["active_tool"] = tool
        controls["pan_tool"].setChecked(tool == "pan")
        controls["inspect_tool"].setChecked(tool == "inspect")
        self._orientation_log(mode, f"Orientation map tool: {tool}.")

    def _orientation_map_clicked(self, mode: str, event) -> None:
        controls = self.orientation_controls.get(mode)
        if controls is None or controls.get("active_tool") != "inspect":
            return
        result = self.orientation_results.get(mode)
        if result is None:
            return
        image_item = controls["image"].getImageItem()
        pos = image_item.mapFromScene(event.scenePos())
        scan_y = int(round(float(pos.x())))
        scan_x = int(round(float(pos.y())))
        shape = result.best_score.shape if isinstance(result, NativeOrientationMapResult) else result.corr.shape
        if not (0 <= scan_y < shape[0] and 0 <= scan_x < shape[1]):
            return
        controls["selected_pixel"] = (scan_y, scan_x)
        self._update_orientation_marker(mode)
        self._update_orientation_inspector(mode)
        event.accept()

    def _update_orientation_marker(self, mode: str) -> None:
        controls = self.orientation_controls[mode]
        selected = controls.get("selected_pixel")
        marker = controls.get("map_marker")
        if marker is None:
            return
        if selected is None:
            marker.setData([])
            return
        scan_y, scan_x = selected
        marker.setData([{"pos": (float(scan_y), float(scan_x))}])

    def _update_orientation_inspector(self, mode: str) -> None:
        controls = self.orientation_controls[mode]
        selected = controls.get("selected_pixel")
        if selected is None:
            return
        y, x = selected
        peaks = self._orientation_peaks_at(y, x)
        if peaks is None:
            self._orientation_log(mode, f"No peaks at selected pixel ({y}, {x}).")
            return
        workspace = self._orientation_workspace(mode)
        if workspace is None:
            return
        ...
        if matches is None:
            matches = match_native_orientation(workspace, peaks)
        if not matches:
            self._orientation_log(mode, f"No native match at selected pixel ({y}, {x}).")
            return
        self._show_native_fit_overlay(mode, workspace, peaks, matches)
        if isinstance(result, NativeOrientationMapResult):
            bary = result.best_zone_barycentric[y, x]
            nearest = int(np.nanargmax(np.nan_to_num(bary, nan=-np.inf))) + 1 if np.isfinite(bary).any() else 0
            label = result.zone_label_topn[y, x, 0] if result.zone_label_topn.size else ""
            self._orientation_log(
                mode,
                f"Inspect ({y}, {x}): best {result.best_score[y, x]:.4g}, "
                f"second {result.second_score[y, x]:.4g}, gap {result.confidence_gap[y, x]:.4g}, "
                f"ratio {result.confidence_ratio[y, x]:.4g}, matched {result.matched_peak_count[y, x]}, "
                f"residual {result.mean_residual_q[y, x]:.4g}, "
                f"zone {label} lattice "
                f"[{result.best_zone_axis_lattice[y, x, 0]:.3g}, {result.best_zone_axis_lattice[y, x, 1]:.3g}, {result.best_zone_axis_lattice[y, x, 2]:.3g}], "
                f"barycentric [{bary[0]:.2g}, {bary[1]:.2g}, {bary[2]:.2g}], nearest vertex {nearest}, "
                f"tilt {result.tilt_deg[y, x]:.3g} deg / azimuth {result.tilt_azimuth_deg[y, x]:.3g} deg.",
            )
    ...
    def _orientation_triangle_legend_image(self, mode: str) -> tuple[np.ndarray, list[str]]:
        ...
        colors = np.asarray([[0.95, 0.12, 0.16], [0.10, 0.58, 0.95], [0.16, 0.72, 0.28]], dtype=np.float64)
    ...
    def _cif_calibration_config(self, mode: str) -> CifCalibrationConfig:
        controls = self.orientation_controls[mode]
        return CifCalibrationConfig(
            qx_center=controls["qx_center"].value(),
            qy_center=controls["qy_center"].value(),
            q_pixel_size=controls["q_pixel_size"].value(),
            flip_qy=controls["flip_qy"].isChecked(),
            detector_rotation_deg=controls["detector_rotation"].value(),
            k_max=controls["k_max"].value(),
            min_q=0.0,
            center_exclusion_q=controls["center_exclusion_q"].value(),
            match_tolerance_q=controls["match_tolerance_q"].value(),
            zone_axis=self._orientation_zone_axis(mode),
            sigma_excitation=controls["sigma_excitation"].value(),
            intensity_weight_mode="uniform" if controls["uniform_template_intensity"].isChecked() else "sqrt",
        )

    def _native_orientation_config(self, mode: str) -> NativeOrientationConfig:
        controls = self.orientation_controls[mode]
        return NativeOrientationConfig(
            mode=mode,
            calibration=self._cif_calibration_config(mode),
            angle_step_in_plane=controls["in_plane_step"].value(),
            angle_step_zone_axis=controls["zone_step"].value(),
            num_matches=controls["num_matches"].value(),
            min_number_peaks=controls["min_peaks"].value(),
            zone_range_mode=(
                "center_cone"
                if mode == "out_of_plane"
                and controls["range_mode"].currentText() == "Center + tilt cone"
                else "three_vertices"
                if mode == "out_of_plane"
                and controls["range_mode"].currentText() == "Three zone-axis vertices"
                else "fixed"
            ...
            use_gpu=controls["use_cuda"].isChecked() and controls["use_cuda"].isEnabled(),
            gpu_chunk_size=controls["gpu_chunk"].value(),
        )
    ...
    def _rotation_symmetry_changed(self, mode: str) -> None:
        ...
        if wheel is not None:
            wheel.setPixmap(
                _color_wheel_pixmap(
                    128,
                    self._rotation_symmetry(mode),
                    angle_sign=self._native_angle_sign(mode),
                    zero_angle_offset_deg=controls["zero_angle_offset"].value(),
                )
            )
        if self.orientation_results.get(mode) is not None:
            self.refresh_orientation_display(mode)
        self._orientation_log(mode, f"Rotation symmetry set to {self._rotation_symmetry(mode)}.")

    def _native_angle_sign(self, mode: str) -> float:
        """Return explicit display sign for native in-plane CIF angles."""

        controls = self.orientation_controls.get(mode, {})
        convention = controls.get("angle_convention")
        text = convention.currentText() if convention is not None else "CIF-positive"
        if text == "CIF-positive":
            return 1.0
        if text == "Detector-screen-positive":
            return -1.0 if self._cif_calibration_config(mode).flip_qy else 1.0
        return -1.0
    ...
    def _out_of_plane_range_mode_changed(self, mode: str) -> None:
        ...
        legend = controls.get("triangle_legend")
        if legend is not None and mode == "out_of_plane":
            image, _labels = self._orientation_triangle_legend_image(mode)
            legend.setPixmap(_array_to_pixmap(image, width=140))
    ...
    def sync_out_of_plane_from_in_plane(self) -> None:
        ...
        self.orientation_vector_calibrations.pop("out_of_plane", None)
    ...
    def _orientation_crystal(self, mode: str):
        ...
        controls = self.orientation_controls[mode]
        try:
            crystal = load_native_crystal(Path(controls["cif_path"].text()))
        except Exception as exc:
            self._orientation_log(mode, f"Failed to load native CIF crystal: {exc}")
            return None
        self.orientation_crystals[mode] = crystal
        self._orientation_log(mode, "Native CIF crystal ready.")
        return crystal
    ...
    def _plot_q_pixel_scan(self, mode: str, scan) -> None:
        ...
        weighted = np.asarray(scan.experimental_profile, dtype=np.float64) * np.asarray(scan.q_pixels, dtype=np.float64)
        sf = np.asarray(scan.cif_profile, dtype=np.float64)
        ...
        plot.plot(scan.cif_q, sf * scale, pen=pg.mkPen("#ff4d4f", width=2), name="CIF radial")
        score_scaled = scan.scores / max(float(np.nanmax(scan.scores)), 1.0) * scale
        q_score = np.interp(
            scan.candidate_pixel_sizes,
            [scan.candidate_pixel_sizes.min(), scan.candidate_pixel_sizes.max()],
            [0, controls["k_max"].value()],
        )
    ...
    def _update_calibration_report(self, mode: str) -> None:
        controls = self.orientation_controls[mode]
        vector = self.orientation_vector_calibrations.get(mode)
        config = self._cif_calibration_config(mode)
        lines = [
            f"qx center: {config.qx_center:.6g}",
            f"qy center: {config.qy_center:.6g}",
            f"q pixel size: {config.q_pixel_size:.8g}",
            f"qy flip: {config.flip_qy}",
            f"detector rotation: {config.detector_rotation_deg:.4g} deg",
            f"match tolerance q: {config.match_tolerance_q:.6g}",
        ]
        radial = self.orientation_radial_calibrations.get(mode)
        if radial is not None:
            top = ", ".join(
                f"{pix:.6g} ({score:.3g})"
                for pix, score in zip(radial.top_pixel_sizes, radial.top_scores)
            )
            lines.extend(
                [
                    f"radial best q pixel: {radial.best_q_pixel_size:.8g}",
                    f"radial top candidates: {top}",
                ]
            )
        if vector is not None:
            lines.extend(
                [
                    f"matched peak count: {vector.matched_peak_count}",
                    f"mean residual q: {vector.mean_residual_q:.6g}",
                    f"median residual q: {vector.median_residual_q:.6g}",
                    f"recommended match tolerance: {vector.recommended_match_tolerance_q:.6g}",
                    f"quality: {vector.quality}",
                    "suggestions:",
                    *[f"- {item}" for item in vector.suggestions],
                ]
            )
        controls["calibration_report"].setPlainText("\n".join(lines))
        self._orientation_log(mode, "Calibration report updated.")

    def _native_matches_from_result_pixel(
        self,
        ...
        matches = []
        for rank in range(result.score_topn.shape[2]):
            score = float(result.score_topn[y, x, rank])
            if not np.isfinite(score):
                continue
            template_idx = int(result.template_index_topn[y, x, rank])
            if template_idx < 0 or template_idx >= len(plan.templates):
                continue
            from fourdlab.processing.native_matching import SparseMatchResult
            ...
            matches.append(
                (
                    ...
                    SparseMatchResult(
                        score=score,
                        matched_peak_count=int(result.matched_peak_count_topn[y, x, rank]),
                        mean_residual_q=float(result.mean_residual_topn[y, x, rank]),
                        median_residual_q=float(result.mean_residual_topn[y, x, rank]),
    ...
    def _find_native_template_index(
        self,
        plan: NativeOrientationPlan,
        ...
        for idx, template in enumerate(plan.templates):
            error = float(np.linalg.norm(template.zone_axis - zone_axis)) + abs(
                ((template.in_plane_angle_deg - float(angle_deg) + 180.0) % 360.0) - 180.0
            ) / 360.0
            if error < best_error:
                best_error = error
                best_idx = idx
    ...
    def _show_native_fit_overlay(self, mode: str, plan: NativeOrientationPlan, peaks: PeakList, matches) -> None:
        controls = self.orientation_controls[mode]
        plot = controls["plot"]
        plot.clear()
        _style_fit_plot(plot)
        ...
        self._orientation_log(
            mode,
            "Native CIF uses cell constants and approximate reflection weights; "
            "atom-site structure factors are not yet enabled.",
        )
        exp_qxy, exp_intensity = peak_list_to_calibrated_qxy(peaks, plan.config.calibration)
        x_values = []
        y_values = []
        if exp_qxy.size:
            plot.plot(
            ...
            x_values.append(exp_qxy[:, 0])
            y_values.append(exp_qxy[:, 1])
        colors = ["#ff3333", "#9b59b6"]
        for rank, (template_idx, match) in enumerate(matches[:2]):
            template = plan.templates[template_idx]
            color = colors[min(rank, len(colors) - 1)]
            name = "best template" if rank == 0 else "second template"
            plot.plot(
                template.qxy[:, 0],
                template.qxy[:, 1],
                pen=None,
                symbol="+" if rank == 0 else "x",
                symbolSize=15,
                symbolPen=pg.mkPen(color, width=2),
                name=f"{name} {match.score:.3g}",
            )
            x_values.append(template.qxy[:, 0])
            y_values.append(template.qxy[:, 1])
            if rank == 0 and exp_qxy.size:
                valid_template, valid_exp = self._draw_native_match_lines(
                    plot,
                    template.qxy,
                    exp_qxy,
                    plan.config.calibration.match_tolerance_q,
                )
                self._add_native_hkl_labels(plot, template, valid_template)
                self._show_unmatched_native_peaks(
                    plot,
                    template,
                    exp_qxy,
                    exp_intensity,
                ...
                self._orientation_log(
                    mode,
                    f"Fit diagnostics: mean residual {match.mean_residual_q:.4g}, "
                    f"median residual {match.median_residual_q:.4g}, "
                    f"q pixel {plan.config.calibration.q_pixel_size:.6g}, "
                    f"rotation {plan.config.calibration.detector_rotation_deg:.3g} deg, "
                    f"qy flip {plan.config.calibration.flip_qy}, "
                    f"center exclusion {plan.config.calibration.center_exclusion_q:.4g}.",
                )
        self._set_square_plot_range(
            plot,
            x_values,
            y_values,
            fallback_radius=float(plan.config.calibration.k_max) + 0.1,
            margin=0.10,
        )
        controls["plot_kind"] = "fit"

    def _draw_native_match_lines(
        self,
        plot,
        ...
        for template_idx in np.flatnonzero(valid)[:80]:
            exp_idx = int(indices[template_idx])
            plot.plot(
                [float(template_qxy[template_idx, 0]), float(exp_qxy[exp_idx, 0])],
                [float(template_qxy[template_idx, 1]), float(exp_qxy[exp_idx, 1])],
                pen=pg.mkPen("#26a269", width=1),
            )
        return np.flatnonzero(valid).astype(np.int64), indices[valid].astype(np.int64)

    def _add_native_hkl_labels(self, plot, template, matched_template_indices: np.ndarray) -> None:
        hkl = template.hkl
        if hkl is None or len(hkl) == 0:
            return
        for idx in matched_template_indices[:20]:
            if idx >= len(hkl):
                continue
            h, k, l = hkl[idx]
            text = pg.TextItem(f"{int(h)}{int(k)}{int(l)}", color="#7c2d12", anchor=(0.5, 1.25))
            text.setPos(float(template.qxy[idx, 0]), float(template.qxy[idx, 1]))
            plot.addItem(text)

    def _show_unmatched_native_peaks(
        self,
        plot,
        template,
        exp_qxy: np.ndarray,
        ...
        if exp_qxy.size:
            unmatched_exp = np.setdiff1d(np.arange(exp_qxy.shape[0]), matched_exp_indices, assume_unique=False)
            if unmatched_exp.size:
                order = unmatched_exp[np.argsort(exp_intensity[unmatched_exp])[::-1]][:8]
                plot.plot(
                    exp_qxy[order, 0],
                    exp_qxy[order, 1],
                    pen=None,
                    symbol="s",
                    symbolSize=9,
                    symbolPen=pg.mkPen("#f59f00", width=1.5),
                    symbolBrush=pg.mkBrush(245, 159, 0, 60),
                    name="unmatched exp",
                )
        unmatched_template = np.setdiff1d(
            np.arange(template.qxy.shape[0]),
            matched_template_indices,
            assume_unique=False,
        )
        if unmatched_template.size:
            strength = np.asarray(template.intensity, dtype=np.float64)
            order = unmatched_template[np.argsort(strength[unmatched_template])[::-1]][:8]
            plot.plot(
                template.qxy[order, 0],
                template.qxy[order, 1],
                pen=None,
                symbol="t",
                symbolSize=9,
                symbolPen=pg.mkPen("#7048e8", width=1.4),
                symbolBrush=pg.mkBrush(112, 72, 232, 50),
...
def _array_to_pixmap(image: np.ndarray, width: int = 160) -> QPixmap:
    arr = np.asarray(image, dtype=np.float64)
    arr8 = np.ascontiguousarray(np.clip(arr * 255.0, 0, 255).astype(np.uint8))
    qimage = QImage(arr8.data, arr8.shape[1], arr8.shape[0], arr8.shape[1] * 3, QImage.Format_RGB888).copy()
