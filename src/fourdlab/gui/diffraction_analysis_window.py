    ...
    def refresh_orientation_display(self, mode: str) -> None:
        ...
        if isinstance(result, NativeOrientationMapResult):
            if label == "Orientation color":
                image = orientation_color_image_native(result, mode)
    ...
    def _native_matches_from_result_pixel(
        ...
        for rank in range(result.score_topn.shape[2]):
            ...
            if not np.isfinite(score):
                continue
            template_idx = self._find_native_template_index(
                plan,
                result.zone_axis_topn[y, x, rank],
                result.in_plane_angle_topn_deg[y, x, rank],
            )
